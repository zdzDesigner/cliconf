local map_fzf = function(mode, key, f, options, buffer)
  ...
  require('util').remap(mode, key, rhs, map_options)
end
...
map_fzf('n', '<C-f>', "live_grep", { resume = false })
...
-- map_fzf('n', '<leader>l', "lsp_document_symbols", { jump_to_single_result = true })
map_fzf('n', '<leader>l', "lsp_document_symbols", { jump_to_symbol_at_cursor = true })
